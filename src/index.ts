...
import {Fetcher} from '@crosswise/sdk'
import {ethers} from 'ethers'

const mainnet = Ethereum.Mainnet
const testnet = Ethereum.Testnet
...
const updateTokensAndPairs = async (
  ...
  pairs.forEach((pair) => {
    ...
    Pairs.watch(
      pair,
      async (syncedPair) => {
        const newPair = await Fetcher.fetchPairData(
          syncedPair.token0,
          syncedPair.token1,
          mainnet.provider
        )
        const index = pairs.findIndex(
          (p) => p.liquidityToken.address === syncedPair.liquidityToken.address
        )
        if (index >= 0) {
