  ...
  async match(
    ...
    for (const order of orders) {
      ...
      if (
        fromToken &&
        toToken &&
        order.deadline.toNumber() * 1000 >= now &&
        filledAmountIn.lt(order.amountIn)
      ) {
        ...
        if (trade) {
          ...
          if (
            deductFee(order.amountOutMin).lt(tradeAmountOutMin.raw.toString())
          ) {
            executables.push({
              ...order,
              trade,
      ...
      args.forEach((arg) => {
        Log.d(
          '  ' +
            arg.order.hash +
            ' (amountIn: ' +
            arg.order.trade.inputAmount.toFixed() +
            ')'
        )
      })
