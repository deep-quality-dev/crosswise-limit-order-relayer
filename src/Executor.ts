import { Pair, Percent, Token, TokenAmount, Trade } from "@crosswise/sdk";
...
const argsForOrder = async (order: Order, signer: ethers.Signer) => {
    const contract = new ethers.Contract(process.env.SETTLEMENT_ADDRESS, SettlementAbi, signer);
    const arg = {
        order,
...
class Executor {
    ...
    watch(onOrderFilled: OnOrderFilled) {
        const settlement = new ethers.Contract(process.env.SETTLEMENT_ADDRESS, SettlementAbi, this.provider);
