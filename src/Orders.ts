...
class Orders {
  private static async fetchCanceledHashes(
    ...
    return (await settlement.queryFilter(filter, fromBlock)).map(
      (event) => event.args![0]
    )
  }

  private static async fetchHashes(
    ...
    return (
      await Promise.all(
        pages.map(async (page) => await orderBook.allHashes(page, LIMIT))
      )
    )
      .flat()
